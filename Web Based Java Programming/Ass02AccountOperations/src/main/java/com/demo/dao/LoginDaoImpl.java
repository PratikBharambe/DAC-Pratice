...
public class LoginDaoImpl implements LoginDao {
	...
	static {
		...
		try {
			selectUserStatement = conn.prepareStatement("select * from user where username = ? and password = ?");
	...
	public MyUser authenticateUser(String username, String password) {
		try {
			...
			if(rs.next()) {
				return new MyUser(rs.getInt(1), rs.getString(2), rs.getString(2), rs.getString(4));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
